...
models/ahicsr.py
----------------
Differentiable AHICSR (Himawari AHI Clear-Sky Radiance, 9 Channels) Forward Operator
...
AHICSR_PEAK_ALTITUDES_M = np.array([
    100.0, 9000.0, 6000.0, 4000.0, 100.0, 10500.0, 100.0, 100.0, 500.0
], dtype=np.float32)

AHICSR_SIGMA_HEIGHTS_M = np.array([
    1200.0, 2600.0, 2200.0, 1800.0, 1000.0, 2800.0, 1000.0, 1000.0, 1400.0
], dtype=np.float32)

AHICSR_PEAK_P_PA = np.array([
    100000.0, 32000.0, 48000.0, 62000.0, 100000.0, 25000.0, 100000.0, 100000.0, 95000.0
], dtype=np.float32)
...
class DifferentiableAHICSROperator(nn.Module):
    def __init__(self, num_levels: int = 32):
        ...
        self.num_channels = len(AHICSR_PEAK_P_PA)

        self.register_buffer("peak_z", torch.from_numpy(AHICSR_PEAK_ALTITUDES_M))
        self.register_buffer("sigma_z", torch.from_numpy(AHICSR_SIGMA_HEIGHTS_M))
