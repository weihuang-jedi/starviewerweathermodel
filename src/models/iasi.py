...
models/iasi.py
--------------
Differentiable IASI Forward Radiance Operator supporting 3D terrain-following heights (h_3d).
"""
...
class DifferentiableIASIOperator(nn.Module):
    def __init__(self, num_levels: int = 32):
        ...
        # Linear altitude spacing from surface (100m) to upper stratosphere (45,000m)
        peak_z_arr = np.linspace(100.0, 45000.0, self.num_channels, dtype=np.float32)
        sigma_z_arr = np.linspace(1200.0, 4800.0, self.num_channels, dtype=np.float32)

        peak_p_arr = np.logspace(np.log10(100000.0), np.log10(100.0), self.num_channels, dtype=np.float32)
        sigma_p_arr = np.full(self.num_channels, 0.35, dtype=np.float32)
        errors_arr = np.linspace(0.8, 1.8, self.num_channels, dtype=np.float32)
